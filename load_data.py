...
def rotate_image(image_to_rotate, rot_ang_deg)
    rotated = ndimage.rotate(image_to_rotate, rot_ang_deg)
    return rotated


def load_data(path_to_data, rot_ang_deg=0.):
...
def load_mnist(rot_ang_deg=0.):
    ...
    if (rot_ang_deg != 0.):
        x_train = rotate_image(x_train, rot_ang_deg)
        x_test = rotate_image(x_test, rot_ang_deg)
